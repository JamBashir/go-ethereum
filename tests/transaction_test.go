...
func TestTransaction(t *testing.T) {
	...
	txt := new(testMatcher)
	// These can not be parsed, invalid hex in RLP
	txt.skipLoad("^ttWrongRLP/.*")
	// We don't allow more than uint64 in gas amount
	// It is a pseudo-consensus vulnerability, but not in practice
	...
	// It is also not a concern, as long as tx.Cost() uses big.Int for
	// calculating the final cozt
	txt.skipLoad(".*TransactionWithGasPriceOverflow.*")

	// The nonce is too large for uint64. Not a concern, it means geth would not
